...
(3-State Startup Flow: Loading -> Music Permission -> App Ready + Local MP3)
=============================================================================
"""
...
# -----------------------------------------------------------------------------
# 3. 3-State Startup Machine & Audio Controller (components.html iframe execution)
# -----------------------------------------------------------------------------
def render_startup_state_machine():
    """Render 3-State Startup Machine via components.html to guarantee 100% JavaScript execution."""
    components.html("""
    <script>
    (function() {
        ...
        // 2. Create Full-Screen Master Overlay on parent document
        const overlay = pDoc.createElement('div');
        overlay.id = 'startupMasterOverlay';
        overlay.style.cssText = 'position:fixed; inset:0; z-index:99999999; background:#060911; display:flex; align-items:center; justify-content:center; font-family:-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; transition:opacity 0.45s ease-out;';
        ...
        overlay.innerHTML = `
            <style>
                ...
                @keyframes stage1FadeOut {
                    ...
                    99% { opacity: 0; transform: scale(0.95); }
                    100% { opacity: 0; transform: scale(0.95); display: none; height:0; overflow:hidden; }
                }
            ...
            <!-- State 1: 3-Second Circular STARTING Loader -->
            ...
            <!-- State 2: Background Music Permission Card -->
            <div id="pStage2" style="display:none; opacity:0; text-align:center; padding:3.5rem 2.5rem; max-width:560px; width:90%; background:radial-gradient(circle at 50% 30%, rgba(26,34,52,0.96) 0%, rgba(12,17,28,0.99) 100%); border:1.5px solid rgba(245,158,11,0.45); border-radius:28px; box-shadow:0 30px 85px rgba(0,0,0,0.85),0 0 55px rgba(245,158,11,0.25);">
                <div style="font-size:2.8rem; color:#f59e0b; margin-bottom:1rem; filter:drop-shadow(0 0 16px rgba(245,158,11,0.7));">🎵</div>
                <div style="font-size:2.4rem; font-weight:600; color:#fefce8; margin-bottom:0.8rem; font-family:serif;">Background Music</div>
                <div style="font-size:1.15rem; line-height:1.7; color:#cbd5e1; margin-bottom:2.2rem; max-width:440px; margin-left:auto; margin-right:auto;">Is kahani ko mehsoos karne ke liye background music enable karein.</div>
                <button id="pEnableMusicBtn" style="font-size:1.1rem; font-weight:600; color:#0b0f19; background:linear-gradient(135deg, #fef08a 0%, #f59e0b 50%, #d97706 100%); border:1px solid rgba(254,240,138,0.4); border-radius:9999px; padding:0.95rem 2.6rem; cursor:pointer; box-shadow:0 10px 30px rgba(245,158,11,0.4); transition:all 0.3s ease;">
        ...
        // After 3.15s, transition to State 2 (Music Permission)
        setTimeout(() => {
            const s1 = pDoc.getElementById('pStage1');
            const s2 = pDoc.getElementById('pStage2');
            if (s1) s1.style.display = 'none';
            if (s2) {
                s2.style.display = 'flex';
                s2.style.flexDirection = 'column';
                s2.style.alignItems = 'center';
                s2.style.animation = 'stage2FadeIn 0.45s ease forwards';
            }
        }, 3150);

        // Attach click listener to button inside parent DOM
        const btn = pDoc.getElementById('pEnableMusicBtn');
        if (btn) {
            btn.onclick = function() {
                try {
                    ...
                    function fade() {
                        ...
                        let frac = Math.min(el / 2000, 1);
                        audio.volume = frac * 0.18;
                        if (frac < 1) requestAnimationFrame(fade);
                    }
...
def render_home_chapter():
    ...
    col1, col2, col3 = st.columns([1.6, 1.5, 1.4])
    with col1:
        if st.button(f"{sec['primary_button']}", key="home_primary_cta"):
...
def render_goodbye_chapter():
    ...
    if not st.session_state.get("show_final_dua", False):
        ...
        with col2:
            if st.button(f"{gb['button_label']}", key="reveal_final_sunset_dua"):
